...
export const pathValidator = (path: string[], includedPaths: (RegExp | string)[], excludedPaths: (RegExp | string)[]): boolean => {
	...
	if (includedPaths.length > 0 && excludedPaths.length > 0) {
		const includedMatch = includedPaths.map(regex => (typeof regex === "string" ? new RegExp(regex) : regex)).find(regex => regex.exec(pathStr))
		const excludeMatch = excludedPaths.map(regex => (typeof regex === "string" ? new RegExp(regex) : regex)).find(regex => regex.exec(pathStr))
