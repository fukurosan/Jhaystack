...
describe("Traversal Strategy", () => {
	...
	describe("Find Values", () => {
		...
		it("Limits results", () => {
			...
			const searchString = "min"
			const result = FIND_VALUES(processedData, searchString, comparisonStrategies, 1)
		...
		it("Finds nested objects", () => {
			const comparisonStrategies = [STARTS_WITH, CONTAINS]
			const searchString = "Nes"
			const result = FIND_VALUES(processedData, searchString, comparisonStrategies)
			expect(result.length).toBe(1)
	...
	describe("Find Nested Objects", () => {
		it("Extracts nested object", () => {
			...
			const result = FIND_NESTED_OBJECTS(processedData, searchString, comparisonStrategies)
			expect(result.length).toBe(1)
		...
		it("Limits results", () => {
			const comparisonStrategies = [CONTAINS]
			const searchString = "min"
			const result = FIND_NESTED_OBJECTS(processedData, searchString, comparisonStrategies, 1)
			expect(result.length).toBe(1)
			expect(result[0].item.firstName).toBe("Benjamin")
		})
		...
		it("Considers arrays as absolute values", () => {
			const arrayInNestedData = [
				{
					someProperty: {
						someOtherProperty: {
							someThirdProperty: [[["Nested Object"], ["Nested Object 2"]]]
						}
					}
				}
			].map(object => {
			...
			const result = FIND_NESTED_OBJECTS(arrayInNestedData, searchString, comparisonStrategies)
			expect(result.length).toBe(1)
	...
	describe("Find Objects", () => {
		it("Returns root object on hit", () => {
			const comparisonStrategies = [CONTAINS]
			const searchString = "obj"
			const result = FIND_OBJECTS(processedData, searchString, comparisonStrategies)
			expect(result.length).toBe(1)
			expect(result[0].item.id).toBe("1")
		})
		...
		it("Limits results", () => {
			const comparisonStrategies = [CONTAINS]
			const searchString = "min"
			const result = FIND_OBJECTS(processedData, searchString, comparisonStrategies, 1)
			expect(result.length).toBe(1)
			expect(result[0].item.firstName).toBe("Benjamin")
		})
