...
export default (itemArray: Item[], searchValue: any, comparisonStrategy: ((term: string, context: any) => number)[], limit?: number): SearchResult[] => {
    ...
    let numberOfFound = 0
    let itemNestedObjectMap = new Map<Item, Map<ObjectLiteral, Shard[]>>()
    let shardNestedPathMap = new Map<Shard, string[]>()
    ...
    for (let strategyIndex = 0; strategyIndex < comparisonStrategy.length; strategyIndex++) {
        ...
        for (let itemIndex = 0; itemIndex < itemArray.length; itemIndex++) {
            ...
            if (!itemNestedObjectMap.has(item)) {
                itemNestedObjectMap.set(
                    ...
                    item.shards.reduce((nestedObjectShardMap: Map<ObjectLiteral, Shard[]>, shard: Shard) => {
                        ...
                        const nestedObject = path.reduce((acc: ObjectLiteral, current) => { return acc[current] }, item.original)
            ...
            for (let [nestedObject, shardArray] of nestedObjectShardMap) {
                const foundShard = shardArray.reduce((bestMatch: IComparisonMatch, shard: Shard) => {
